...
const inverse = require('./util/relationship-inverse');
...
function setNonHosts(resource, key, resources) {
  ...
  _.each(relationships, relation => {
    ...
    // at all.
    const inverseRelationship = inverse(relation.relationship);
    const inverseExists = _.find(related.relationships, r => {
      return r.resource === resource.name && r.relationship === inverseRelationship;
    });
