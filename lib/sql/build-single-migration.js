...
function buildAssociativeTable({relationship, definition, definitions}) {
  ...
  const tableName = manyToManyUtil.getAssociativeTableName({host, guest, escaped: true});
  const hostIdColumnName = manyToManyUtil.getHostIdColumnName({host, escaped: true});
  const guestIdColumnName = manyToManyUtil.getGuestIdColumnName({guest, escaped: true});
  const primaryKeyColumnName = manyToManyUtil.getPrimaryKeyColumnName({host, guest, escaped: true});
...
module.exports = function(definition, index, definitions) {
  ...
  // Remove non-hosted relationships. Those are useful for the API, but they are
  // not reflected in the database.
  const hostRelationships = _.pickBy(definition.relationships, {host: true});
  const relationships = _.map(hostRelationships, relationship => generateRelationRow({relationship, definition, definitions}));
  ...
  const associativeTableQueries = _.map(associativeRels, relationship => buildAssociativeTable({
